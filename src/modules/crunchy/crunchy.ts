...
export default class Crunchyroll {
    ...
    private async generateRSSFeed(lang: string) {
        ...
        episodes = episodes.sort((a, b) => {
            const dateA = new Date(a.episode_metadata.availability_starts ?? now).getTime()
            const dateB = new Date(b.episode_metadata.availability_starts ?? now).getTime()
            return dateB - dateA
        })
            ...
            rss: {
                ...
                channel: {
                    ...
                    item: episodes.slice(0, 50).map((ep) => ({
                        ...
                        // Times
                        pubDate: new Date(ep.episode_metadata.availability_starts ?? ep.episode_metadata.episode_air_date).toUTCString(),
        ...
        if (lang === 'en-US' && server.discorditems) {
            await server.discorditems(
                episodes.slice(0, 100).map((ep) => ({
                    ...
                    released: new Date(ep.episode_metadata.availability_starts ?? ep.episode_metadata.episode_air_date),
                    img: (() => {
